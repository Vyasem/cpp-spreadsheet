...
public:
    ...
    virtual void InvalidateCellCache(Position pos) = 0;
    ...
    virtual void CreateDependencies(Position from, const std::vector<Position>& dependencies) = 0;

    //Запуск поиска циклических зависимосте из метода Cell::Set и Cell::Clear
    virtual void FindCyclicDependencies(Position pos) = 0;
    //Запуск инвалидации кеша из метода Cell::Set и Cell::Clear
    virtual  void InvalidateCache(Position pos) = 0;
};
